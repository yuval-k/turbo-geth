...
func (ig *IndexGenerator) GenerateIndex(blockNum uint64, changeSetBucket []byte) error {
	csInfo, ok := CSMapper[string(changeSetBucket)]
	if !ok {
	...
	err := etl.Transform(
		...
		csInfo.IndexBucket,
		ig.TempDir,
		getExtractFunc(csInfo.WalkerAdapter),
		loadFunc,
		etl.TransformArgs{Quit: ig.quitCh, ExtractStartKey: dbutils.EncodeTimestamp(blockNum)},
	)
	if err != nil {
		return err
	}
...
func loadFunc(k []byte, valueDecoder etl.Decoder, state etl.State, next etl.LoadNextFunc) error { //nolint
	...
	fmt.Println("core/generate_index.go:425 loadFunc", common.Bytes2Hex(k), blockNumbers, k)
	if err != nil {
		return err
	}
