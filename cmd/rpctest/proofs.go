...
func proofs(chaindata string, url string, block int) {
	...
	if _, err := os.Stat(fileName); err != nil {
		if os.IsNotExist(err) {
			...
			subTries, err1 := l.LoadSubTries(ethDb, trie.NewTrie2(), uint64(block), rl, nil /* HashCollector */, [][]byte{nil}, []int{0}, false)
			if err1 != nil {
				panic(err1)
			}
...
func fixState(chaindata string, url string) {
	...
	for addrHash, account := range roots {
		if account != nil && account.Root != trie.EmptyRoot {
			...
			subTries, err1 := sl.LoadSubTries(stateDb, trie.NewTrie2(), blockNum, rl, nil /* HashCollector */, [][]byte{contractPrefix}, []int{8 * len(contractPrefix)}, false)
