...
func checkRoots(stateDb ethdb.Database, rootHash common.Hash, blockNum uint64) {
	...
	if blockNum > 0 {
		...
		subTries, err := l.LoadSubTries(stateDb, trie.NewTrie2(), blockNum, rl, nil /* HashCollector */, [][]byte{nil}, []int{0}, false)
		if err != nil {
			fmt.Printf("%v\n", err)
		}
	...
	for addrHash, account := range roots {
		if account != nil {
			...
			rl := trie.NewRetainList(0)
			subTries, err := sl.LoadSubTries(stateDb, trie.NewTrie2(), blockNum, rl, nil /* HashCollector */, [][]byte{contractPrefix}, []int{8 * len(contractPrefix)}, false)
			if err != nil {
				fmt.Printf("%x: %v\n", addrHash, err)
