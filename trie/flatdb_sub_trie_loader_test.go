...
func TestResolve1(t *testing.T) {
	...
	subTries, err := r.LoadSubTries(db, NewTrie2(), 0, rs, nil /* HashCollector */, [][]byte{common.Hex2Bytes("aaaaabbbbb")}, []int{40}, false)
	require.NoError(err)
	tr := New(common.Hash{})
...
func TestResolve2(t *testing.T) {
	...
	rs := NewRetainList(0)
	rs.AddKey(common.Hex2Bytes("aaaaabbbbbaaaaabbbbbaaaaabbbbbaa"))
	subTries, err := r.LoadSubTries(db, NewTrie2(), 0, rs, nil /* HashCollector */, [][]byte{common.Hex2Bytes("aaaaaaaaaa")}, []int{40}, false)
	require.NoError(err)
...
func TestTrieSubTrieLoader(t *testing.T) {
	...
	_, err := resolver.LoadSubTries(db, NewTrie2(), 0, rs, nil, /* HashCollector */
...
func TestTwoStorageItems(t *testing.T) {
	...
	resolver := NewSubTrieLoader(0)
	rs := NewRetainList(0)
	subTries, err1 := resolver.LoadSubTries(db, NewTrie2(), 0, rs, nil /* HashCollector */, [][]byte{nil}, []int{0}, false)
	...
	rs2 := NewRetainList(0)
	rs2.AddHex([]byte{0xd})
	subTries, err = resolver2.LoadSubTries(db, NewTrie2(), 0, rs2, nil /* HashCollector */, [][]byte{{0xd0}}, []int{4}, false)
	require.NoError(err, "resolve error")
...
func TestApiDetails(t *testing.T) {
	...
	*/
...

	{
		...
		subTries, err := loader.LoadSubTries(db, NewTrie2(), 0, rs, nil /* HashCollector */, [][]byte{nil}, []int{0}, false)
		assert.NoError(err)
	...
	{ // storage loader
		...
		rl.Rewind()
		subTries, err := loader.LoadSubTries(db, NewTrie2(), 0, rl, nil /* HashCollector */, dbPrefixes, fixedbits, false)
		require.NoError(err)
...
func TestIsBefore(t *testing.T) {
	...
	is, minKey := keyIsBeforeOrEqual([]byte("a"), []byte("b"))
	assert.Equal(true, is)
	assert.Equal("a", fmt.Sprintf("%s", minKey))

	is, minKey = keyIsBeforeOrEqual([]byte("b"), []byte("a"))
	assert.Equal(false, is)
	assert.Equal("a", fmt.Sprintf("%s", minKey))

	is, minKey = keyIsBeforeOrEqual([]byte("b"), []byte(""))
	assert.Equal(false, is)
	assert.Equal("", fmt.Sprintf("%s", minKey))

	is, minKey = keyIsBeforeOrEqual(nil, []byte("b"))
	assert.Equal(false, is)
	assert.Equal("b", fmt.Sprintf("%s", minKey))

	is, minKey = keyIsBeforeOrEqual([]byte("b"), nil)
	assert.Equal(true, is)
	assert.Equal("b", fmt.Sprintf("%s", minKey))
	...
	is, minKey = keyIsBeforeOrEqual(cacheKey, storageKey)
	assert.False(is)
