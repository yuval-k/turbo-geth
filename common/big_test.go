		...
		for l := n; l <= 4; l++ {
			...
			for _, b := range bs {
				for i := range slice {
					...
					withoutPaddingDirect := setUint256WithoutPadding(slice)
					...
					if withoutPadding.Cmp(withoutPaddingDirect) != 0 {
						t.Fatalf("not equal n=%d l=%d slice=%v\nGot\t\t%v\n!=\nExpect\t%v(%v)",
							n, l, slice, withoutPadding.Bytes32(), withoutPaddingDirect.Bytes32(), spew.Sdump(withoutPaddingDirect))
					}
				}
			}
		}
...
func setUint256WithoutPadding(slice []byte) *uint256.Int {
	return new(uint256.Int).SetBytes(slice)
}
